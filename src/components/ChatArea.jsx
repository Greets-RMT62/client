...
export default function ChatArea({
  ...
  const handleSummaryGenerated = (summaryText) => {
    ...
    setChats((prevChats) =>
      prevChats.map((chat) =>
        chat.id === localActiveChat.id ? { ...chat, messages: [...chat.messages, summaryMessage] } : chat
      )
    );
  ...
  useEffect(() => {
    if (localActiveChat) {
      console.log('ChatArea - Active chat messages updated:', localActiveChat.messages);
    }
  }, [localActiveChat]);
    ...
    <main className="flex-1 flex flex-col">
      <header
        className={`p-6 ${
          isDarkMode
            ...
            : 'bg-white/50 backdrop-blur-xl border-white/20 text-gray-900'
        ...
        <div className="flex items-center gap-4">
          {localActiveChat && (
            <>
              ...
              <div>
                <h2 className={`font-bold text-xl ${isDarkMode ? 'text-white' : ''}`}>
                ...
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} flex items-center gap-2`}>
                  {localActiveChat.isGroup ? (
                    <>
                      <Users size={14} />
                      <span>{localActiveChat.messages.filter((m) => m.from !== 'System').length} members</span>
                    </>
                  ) : (
                    <span>{localActiveChat.lastSeen}</span>
                  )}
                  {Object.entries(typingUsers).some(([userId, isTyping]) => isTyping && userId !== USER_ID) && (
                    <span className="text-green-500 animate-pulse">
                      •{' '}
                      {Object.entries(typingUsers)
                        .filter(([userId, isTyping]) => isTyping && userId !== USER_ID)
                        .map(([userId]) => userId)
                        .join(', ')}{' '}
                      {Object.entries(typingUsers).filter(([userId, isTyping]) => isTyping && userId !== USER_ID)
                        .length > 1
                        ? 'are'
                        : 'is'}{' '}
                      typing...
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-3 rounded-2xl transition-all duration-300 hover:scale-110 ${
                isDarkMode ? 'hover:bg-blue-700/20 text-blue-400' : 'hover:bg-blue-50 text-blue-600'
              }`}
            ...
            <button
              ...
              className={`p-3 rounded-2xl transition-all duration-300 hover:scale-110 ${
                isDarkMode ? 'hover:bg-gray-700 text-green-400' : 'hover:bg-green-50 text-green-600'
              }`}
            >
              <Phone size={20} />
            </button>
      ...
      <div
        ...
        className={`flex-1 overflow-y-auto p-6 space-y-4 ${
          isDarkMode ? 'bg-gray-950' : 'bg-gradient-to-b from-purple-50/30 to-pink-50/30'
        }`}
      >
                ...
                <div key={index} className="flex justify-center">
                  <div
                    className={`px-4 py-2 rounded-full text-xs ${
                      isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-white/70 text-gray-500'
                    } backdrop-blur-sm`}
                  >
                    {msg.text}
                  </div>
                ...
                <div key={index} className="flex justify-start mb-4">
                  <div className="flex items-start gap-3 max-w-xs md:max-w-md lg:max-w-lg">
                    ...
                    >
                      ...
                      <div className={`text-sm leading-relaxed ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>
                        {msg.text}
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className={`max-w-xs md:max-w-md lg:max-w-lg ${isMe ? 'order-2' : 'order-1'}`}>
                  ...
                  {msg.replyTo && (
                    ...
                    >
                      <div className={`font-medium text-xs mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        Replying to {msg.replyTo.from}
                      </div>
                      <div className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} truncate`}>
                        {msg.replyTo.text}
                      </div>
                    </div>
                  )}
                  ...
                  <div className={`flex items-center gap-2 mt-2 px-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                    ...
                    {isMe && <CheckCircle2 size={12} className="text-green-400" />}
                  </div>
                </div>
              </div>
            );
            ...
            <div className="text-center">
              {localActiveChat ? (
                ...
                <>
                  ...
                  <p className={`text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} max-w-md mx-auto`}>
                    Select a chat from the sidebar to start messaging, or create a new chat to connect with your team.
                  </p>
                </>
              )}
            </div>
      ...
      {localActiveChat && (
        <div
          className={`p-6 ${
            isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white/50 backdrop-blur-xl border-white/20'
          } border-t`}
        >
          {/* Reply indicator */}
          {replyingTo && (
            <div
              className={`mb-4 p-4 rounded-2xl ${
                isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-purple-50 border border-purple-200'
              } backdrop-blur-sm transition-all duration-300`}
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className={`text-sm font-medium mb-1 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`}>
                    Replying to {replyingTo.from}
                  </div>
                  <div className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} text-sm`}>{replyingTo.text}</div>
                </div>
                <button
                  onClick={cancelReply}
                  className={`p-2 rounded-xl transition-all duration-300 hover:scale-110 ${
                    isDarkMode ? 'hover:bg-gray-600 text-gray-400' : 'hover:bg-red-100 text-gray-500 hover:text-red-500'
                  }`}
                >
                  <X size={16} />
                </button>
          ...
          <div
            ...
            } backdrop-blur-xl shadow-2xl border ${isDarkMode ? 'border-gray-600' : 'border-white/30'}`}
          >
            <div className="relative dropdown-trigger">
              <button
                onClick={() => toggleDropdown('attachDropdown')}
                className={`p-3 rounded-2xl transition-all duration-300 hover:scale-110 ${
                  isDarkMode ? 'hover:bg-gray-600 text-gray-300' : 'hover:bg-purple-100 text-purple-600'
                }`}
              >
                <Paperclip size={20} />
              </button>
            ...
            <button
              className={`p-3 rounded-2xl transition-all duration-300 hover:scale-110 ${
                isDarkMode ? 'hover:bg-gray-600 text-gray-300' : 'hover:bg-yellow-100 text-yellow-600'
              }`}
            >
              <Smile size={20} />
            </button>
            ...
            <input
              ...
              className={`flex-1 bg-transparent border-0 focus:outline-none text-lg placeholder-opacity-70 ${
                isDarkMode ? 'text-white placeholder-gray-300' : 'text-gray-900 placeholder-gray-500'
              }`}
